...
def get_local_distance_minimizing_permutation(x):
    ...
    permutation = torch.tensor(tour[tour.index(n_vectors)+1:] + tour[:tour.index(n_vectors)])
    return permutation

@cache
def get_seriation_permutations(x):
    perm_1, perm_2 = get_local_distance_minimizing_permutation(x), get_local_distance_minimizing_permutation(x.T)
